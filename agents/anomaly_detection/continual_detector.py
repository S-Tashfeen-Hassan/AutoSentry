...
NORMALIZED_LOG_FILE = Path("../../data/normalized_logs.ndjson")
DETECTION_OUTPUT_FILE = Path("../../data/detection_output.ndjson")
SLEEP_TIME = 1.0

def extract_values_from_log(log_dict, target_fields_order):
    """
    Convert a log dict to a list of values in the same order as used in training.
    """
    return [log_dict.get(k, 0) for k in target_fields_order]

def write_ndjson_rows(filepath, rows):
    """Append a list of dict rows to an NDJSON file."""
    with open(filepath, 'a', encoding='utf-8') as f:
        for r in rows:
            # Convert any NumPy arrays in dict to list
            r_copy = {k: (v.tolist() if hasattr(v, 'tolist') else v) for k, v in r.items()}
            f.write(ujson.dumps(r_copy) + '\n')
...
if __name__ == "__main__":
    ...
    print("[INFO] Continual Detector initialized.")

    # Keep track of file read position
    last_pos = NORMALIZED_LOG_FILE.stat().st_size if NORMALIZED_LOG_FILE.exists() else 0

    while True:
        if not NORMALIZED_LOG_FILE.exists():
            print(f"[WARN] Normalized logs file not found: {NORMALIZED_LOG_FILE}")
            time.sleep(1)
            continue

        with open(NORMALIZED_LOG_FILE, 'r', encoding='utf-8') as f:
            f.seek(last_pos)
            lines = f.readlines()
            last_pos = f.tell()  # update last position
        ...
        for line in lines:
            try:
                norm_log = json.loads(line)
            except json.JSONDecodeError:
                continue

            # Convert normalized log dict into a feature array for the detector
            feature_array = [norm_log.get(k, 0) for k in sorted(norm_log.keys())]  # adjust ordering if needed

            # Run detection
            result = detector.process_single(feature_array, verify_normal=False)

            # Combine normalized log and detection result
            combined_record = {
                "normalized_log": norm_log,
                "detection_result": {k: (v.tolist() if hasattr(v, 'tolist') else v) for k, v in result.items()}
            }

            print("[RESULT]", combined_record)  # full output to terminal

            # Write combined record to detection output
            with open(DETECTION_OUTPUT_FILE, 'a', encoding='utf-8') as f:
                f.write(ujson.dumps(combined_record) + '\n')

        # Optional: periodic maintenance & save state
